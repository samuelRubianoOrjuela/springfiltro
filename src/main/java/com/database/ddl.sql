...
CREATE TABLE region (
    ...
    CONSTRAINT FK_code_country_region FOREIGN KEY(codecountry) REFERENCES country(codecountry)
);
...
CREATE TABLE city (
    ...
    namecity VARCHAR(20),
    codereg VARCHAR(5),
    CONSTRAINT FK_code_region_city FOREIGN KEY(codereg) REFERENCES region(codereg)
);
...
CREATE TABLE labatory (
    ...
    CONSTRAINT FK_id_city_lab FOREIGN KEY (codecityreg) REFERENCES city(codecity)
);
